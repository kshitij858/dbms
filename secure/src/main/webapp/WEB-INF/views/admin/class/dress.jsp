...
<body>
	<form  action = "addDress" method = "get">
		 <div class="form-group">
   			<label for="">Add Uniform</label>
   			<select name = "dressId" class = "form-control">
   				<c:forEach var = "one" items = "${options }" >
   					<option value = "${one.dressId}">
   					${one.dressId } ${one.dressDescription } </option>
   				</c:forEach>
   			</select>
	  	</div>
	  	<button type = "submit" class = "btn btn-success" >Add this uniform</button>
	</form>
